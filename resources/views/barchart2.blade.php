...
<html lang="en">
  ...
  <body>
    @php
    use App\Store_area;
    $area = Store_area::where('area_id',$selectarea)->first();
    @endphp
            ...
            @php
              use App\Report_product;
                  $compliance = Report_product::join('store','report_product.store_id','=','store.store_id')
                              ->whereBetween('report_product.tanggal',[$start,$end])
                              ->where('store.area_id',$selectarea)->sum('compliance');
                              
                  $row = Report_product::join('store','report_product.store_id','=','store.store_id')
                              ->whereBetween('report_product.tanggal',[$start,$end])
                              ->where('store.area_id',$selectarea)->COUNT()->get();
                  $data = $row->count();
                  $persen = 100;
